...
namespace NHibernate.Tool.hbm2ddl
{
    ...
    public class SchemaExportExt
    {
        ...
        public SchemaExportExt(Configuration cfg) : this(cfg, cfg.Properties)
        {
        }
        ...
        public SchemaExportExt(Configuration cfg, IDictionary<string, string> configProperties)
        {
            this.cfg = cfg;
        ...
        private void Initialize()
        {
            ...
            string autoKeyWordsImport = PropertiesHelper.GetString(Environment.Hbm2ddlKeyWords, configProperties,
                                                                   "not-defined");
        ...
        public SchemaExportExt SetOutputFile(string filename)
        {
            outputFile = filename;
            return this;
        }
        ...
        /// <returns>The SchemaExport object.</returns>
        public SchemaExportExt SetDelimiter(string delimiter)
        {
        ...
        /// </summary>
        /// <param name="script">
        ///     <see langword="true" /> if the ddl should be outputted in the Console.
        /// </param>
        /// <param name="export">
        ///     <see langword="true" /> if the ddl should be executed against the Database.
        /// </param>
        /// <remarks>
        ...
        public void Create(Action<IDbCommand> beforeExecute, Action<IDbCommand> afterExecute, bool export)
        {
            Execute(beforeExecute, afterExecute, export, false);
        }
        ...
        private void Execute(Action<IDbCommand> beforeExecute, Action<IDbCommand> afterExecute, bool export, bool throwOnError, TextWriter exportOutput,
                             IDbCommand statement, string sql)
        {
            ...
            try
            {
                ...
                string formatted = formatter.Format(statement.CommandText);
                ...
                if (export)
                {
                    ExecuteSql(statement, statement.CommandText);
            ...
            {
                log.Warn("Unsuccessful: " + statement.CommandText);
                log.Warn(e.Message);
                if (throwOnError)
                {
                    throw;
        ...
        private void ExecuteSql(IDbCommand cmd, string sql)
        {
            ...
            {
                IObjectsFactory objFactory = Environment.BytecodeProvider.ObjectsFactory;
                var splitter = (ScriptSplitter) objFactory.CreateInstance(typeof (ScriptSplitter), sql);
        ...
        /// </param>
        /// <param name="justDrop">
        ///     <see langword="true" /> if only the ddl to drop the Database objects should be executed.
        /// </param>
                            ...
                            TextWriter exportOutput)
        {
            if (script)
            {
                Execute(null, Console.WriteLine, export, justDrop, connection, exportOutput);
            }
            else
            {
                Execute(null, null, export, justDrop, connection, exportOutput);
            }
        }

        public void Execute(Action<IDbCommand> beforeExecute, Action<IDbCommand> afterExecute, bool export, bool justDrop, IDbConnection connection, TextWriter exportOutput)
        {
            Initialize();
            ...
            if (export && connection == null)
            {
                throw new ArgumentNullException("connection",
                                                "When export is set to true, you need to pass a non null connection");
            }
            if (export)
            {
            ...
            try
            {
                ...
                {
                    Execute(beforeExecute, afterExecute, export, false, exportOutput, statement, dropSQL[i]);
                }
                ...
                if (!justDrop)
                {
                    ...
                    {
                        Execute(beforeExecute, afterExecute, export, true, exportOutput, statement, createSQL[j]);
                    }
                }
            }
            finally
        ...
        public void Execute(bool script, bool export, bool justDrop)
        {
            if (script)
            {
                Execute(null, Console.WriteLine, export, justDrop);
            }
            else
            {
                Execute(null, null, export, justDrop);
            }
        }
        ...
        public void Execute(Action<IDbCommand> beforeExecute, Action<IDbCommand> afterExecute, bool export, bool justDrop)
        {
            ...
            try
            {
                ...
                Execute(beforeExecute, afterExecute, export, justDrop, connection, fileOutput);
            }
