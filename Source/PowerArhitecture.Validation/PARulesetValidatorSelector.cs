...
namespace PowerArhitecture.Validation
{
    ...
    public class PARulesetValidatorSelector : IValidatorSelector
    {
		...
		public bool CanExecute(IValidationRule rule, string propertyPath, ValidationContext context) {
            if (string.IsNullOrEmpty(rule.RuleSet) && _rulesetsToExecute.Count == 0) return true;
            if (string.IsNullOrEmpty(rule.RuleSet) && _rulesetsToExecute.Count > 0 && _rulesetsToExecute.Contains("default", StringComparer.OrdinalIgnoreCase)) return true;
            if (!string.IsNullOrEmpty(rule.RuleSet) && _rulesetsToExecute.Count > 0 && _rulesetsToExecute.Contains(rule.RuleSet)) return true;
